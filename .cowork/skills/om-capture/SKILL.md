...
> [!note] Provisional ¹
> The discovery path (`.cowork/skills/`) and skill frontmatter schema are best-effort because Copilot Cowork does not publish a dedicated skill discovery convention at the time of writing. If your version expects a different path or schema, adapt this file but keep the prompt body aligned with `.claude/skills/om-capture/SKILL.md`.

> Keep this file in sync with `.claude/skills/om-capture/SKILL.md`.

> [!important]
> Copilot Cowork can run multiple delegated sessions in parallel. **Do not let capture writes from one session race with another session's edits.** Re-read the target note before writing (never write from a stale read), and avoid writing to `work/active/` notes that another session may be actively editing. When in doubt, ask the user before writing.
