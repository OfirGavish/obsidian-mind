...
| 훅 | 시점 | 동작 |
|------|------|------|
| 🚀 SessionStart | 시작/재개 시 | QMD 재인덱싱＋셀프힐, North Star 포커스·활성 작업·최근 변경사항·작업·파일 목록·볼트 위생 플래그 주입 — 전체는 바이트 예산으로 제한되고, 마지막 줄은 주입 크기 미터 |
...
| 🏁 Stop | 세션 종료 시 | 체크리스트＋구체적인 드리프트 발견 (SessionStart와 동일한 위생 스캔) |
...
| 계층 | 내용 | 시점 | 비용 |
|------|------|------|------|
| **항상** | `CLAUDE.md` + SessionStart 컨텍스트 (North Star 발췌, git 요약, 작업, 볼트 파일 목록) | 세션 시작 시 | 매니페스트 예산이 상한. 실제 크기는 매 세션 마지막 줄의 미터가 보고 |
