...
> [!note] Provisional
> The discovery path (`.hermes/skills/`) and skill frontmatter schema (`name`, `description`) are best-effort because Hermes's published native skill documentation is still incomplete. If your version expects a different path or schema, adapt this file but keep the prompt body aligned with `.claude/skills/om-capture/SKILL.md`.

> Keep this file in sync with `.claude/skills/om-capture/SKILL.md`.

> [!important]
> Hermes skill memory is ephemeral. The durable part of the lesson survives only when you record it into `[[Patterns]]`, `[[Gotchas]]`, `[[Key Decisions]]`, `[[Brag Doc]]`, `org/people/`, or the active project note. This skill is the bridge from Hermes's session-only memory to the vault's long-term memory.
