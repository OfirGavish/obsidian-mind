...
> [!note] Provisional ¹
> The discovery path (`.agents/skills/`) and skill frontmatter schema (`name`, `description`) follow the Codex Skills convention documented in [Codex Skills docs](https://developers.openai.com/codex/skills). If your version expects a different path or schema, adapt this file but keep the prompt body aligned with `.claude/skills/om-capture/SKILL.md`.
